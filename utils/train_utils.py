...
def pretext_train(train_loader, model, criterion, optimizer, epoch, prev_loss, device='cuda'):
    ...
    for i, batch in enumerate(train_loader):
        ts_org = batch['ts_org'].float().to(
            device, 
            non_blocking=True
        )

        ts_w_augment = batch['ts_w_augment'].float().to(
            device, 
            non_blocking=True
        )

        ts_ss_augment = batch['ts_ss_augment'].float().to(
            device, 
            non_blocking=True
        )
        ...
        input_: Tensor = torch.cat(
            [ts_org, ts_w_augment, ts_ss_augment],
            dim=0
            ).view(b * 3, h, w)
    ...
    # If training loop didn't run, return the average meter (0.0) instead
    try:
        return loss
