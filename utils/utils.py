...
def fill_ts_repository(p, loader, model, ts_repository, real_aug=False, ts_repository_aug=None):
    ...
    for i, batch in enumerate(loader): 
        ...
        # ts_org shape should be [batch, sequence_length, features]
        print("ts_org.shape:", ts_org.shape)

        if ts_org.dim() != 3:
            raise ValueError(
                f"Expected 3D tensor [batch, seq_len, features], "
                f"but got shape {ts_org.shape}"
            )

        b, seq_len, n_features = ts_org.shape

        print(f"batch={b}, seq_len={seq_len}, features={n_features}")
        ...
        if real_aug:
            ...
            print("\n===== AUGMENT DEBUG =====")
            print("ts_org.shape      :", ts_org.shape)
            print("ts_w_augment.shape:", ts_w_augment.shape)
            print("model expected    :", model.backbone.lstm.input_size)
            print("=========================\n")

            output = model(ts_w_augment)
            ts_repository.update(output, targets)
            ...
            output = model(ts_ss_augment)
            print("ts_ss_augment.shape:", ts_ss_augment.shape)
