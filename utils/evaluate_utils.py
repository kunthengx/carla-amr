...
def pr_evaluate_multi_class(predictions, compute_confusion_matrix=False, majority_label=None):
    """Evaluasi untuk multi-kelas (Normal, Defect, Theft)"""
    ...
    for batch in predictions:
        all_preds.extend(batch['predictions'].cpu().numpy())
        all_targets.extend(batch['targets'].cpu().numpy())
    
    # Classification report
    report = classification_report(all_targets, all_preds, 
                                   target_names=['Normal', 'Defect', 'Theft'],
                                   output_dict=True)
    
    # Macro F1 untuk multi-class
    macro_f1 = f1_score(all_targets, all_preds, average='macro')
    weighted_f1 = f1_score(all_targets, all_preds, average='weighted')
    
    if compute_confusion_matrix:
        cm = confusion_matrix(all_targets, all_preds)
        return report, macro_f1, weighted_f1, cm
