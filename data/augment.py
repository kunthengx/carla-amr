...
def __call__(self, X):
        # ===== VALIDASI INPUT =====
        # Jika X adalah string, raise error yang jelas
        if isinstance(X, str):
            raise TypeError(
                f"Transform received string instead of tensor! "
                f"String value: {X[:100]}... "
                f"Please check your dataset __getitem__ method."
            )
        
        # Jika X adalah numpy array, konversi ke tensor
        if isinstance(X, np.ndarray):
            X = torch.from_numpy(X).float()
        
        # Jika X bukan tensor, coba konversi
        if not isinstance(X, torch.Tensor):
            try:
                X = torch.tensor(X, dtype=torch.float32)
            except:
                raise TypeError(f"Cannot convert {type(X)} to tensor: {X}")
        
        # ===== SEKARANG AMAN UNTUK AKSES .device =====
        if X.device.type == 'cuda':
            # ... kode untuk GPU
            pass
        
        # Lanjutkan transformasi lainnya
        return X
